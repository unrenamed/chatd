...
use crate::server::room::{message, Command, Theme, TimestampMode, User, UserStatus};
...
async fn execute_command(user: User, cmd: Command, terminal: &mut Terminal, room: &mut ServerRoom) {
    ...
    match cmd {
        ...
        Command::Name(new_name) => 'label: {
            ...
            if user.username == new_name {
                let message =
                    message::Error::new(user, "new name is the same as the original".to_string());
                room.send_message(message.into()).await;
                break 'label;
            }
            ...
            if let Some(_) = room.try_find_member(&new_name) {
                let message =
                    message::Error::new(user, format!("\"{}\" name is already taken", new_name));
                room.send_message(message.into()).await;
                break 'label;
            }

            let message =
                message::Announce::new(user.clone(), format!("user is now known as {}.", new_name));
            room.send_message(message.into()).await;
        ...
        Command::Msg(to, msg) => 'label: {
            ...
            match room.try_find_member_mut(&to).map(|a| &mut a.user) {
                None => {
                    let message = message::Error::new(from.clone(), format!("user is not found"));
                    room.send_message(message.into()).await;
                    break 'label;
                }
                Some(to) if from.id.eq(&to.id) => {
                    let message =
                        message::Error::new(from.clone(), format!("you can't message yourself"));
                    room.send_message(message.into()).await;
                    break 'label;
                }
                Some(to) => {
                    ...
                    let message = message::Private::new(from.clone(), to.clone(), msg.to_string());
                    room.send_message(message.into()).await;
                    ...
                    match status {
                        UserStatus::Away { reason, since: _ } => {
                            let message = message::System::new(
                                from.clone(),
                                format!("Sent PM to {}, but they're away now: {}", name, reason),
                            );
                            room.send_message(message.into()).await;
                        }
                        UserStatus::Active => {}
        ...
        Command::Slap(target_name) => 'label: {
            ...
            if target_name.is_none() {
                let message =
                    message::Emote::new(user, "hits himself with a squishy banana.".to_string());
                room.send_message(message.into()).await;
                break 'label;
            }
            ...
            let message = if let Some(t) = target {
                ...
                message::Error::new(user, "that slippin' monkey not in the room".to_string()).into()
            };
            room.send_message(message).await;
        }
        Command::Shrug => {
        ...
        Command::Ignore(target) => 'label: {
                ...
                .map(|a| a.user.id.clone())
            {
                Some(target_id) if target_id == user.id => {
                    let message =
                        message::Error::new(user.clone(), "you can't ignore yourself".to_string());
                    room.send_message(message.into()).await;
                    break 'label;
                }
                ...
                None => {
                    let message = message::Error::new(user.clone(), "user not found".to_string());
                    room.send_message(message.into()).await;
                    break 'label;
                }
                Some(target_id) => {
        ...
        Command::Mute(target_username) => 'label: {
                ...
                .map(|a| &mut a.user)
            {
                ...
                Some(target) if target.id == user.id => {
                    let message = message::Error::new(user, "you can't mute yourself".to_string());
                    room.send_message(message.into()).await;
                    break 'label;
                }
                Some(target) => {
        ...
        Command::Motd(new_motd) => 'label: {
            ...
            if !user.is_op {
                let message =
                    message::Error::new(user, "must be an operator to modify the MOTD".to_string());
                room.send_message(message.into()).await;
                break 'label;
            }
